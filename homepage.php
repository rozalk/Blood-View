...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $recordDate = mysqli_real_escape_string($con, $_POST['recordDate']);
    $bloodPints = mysqli_real_escape_string($con, $_POST['bloodPints']);

    // Check if withdrawal is possible based on available blood stock
    $checkStockQuery = "SELECT quantity FROM blood_stock WHERE blood_type = '$bloodType'";
    ...
    if ($checkStockResult && mysqli_num_rows($checkStockResult) > 0) {
        ...
        if ($bloodPints <= $availableQuantity) {
            // Perform withdrawal
            $withdrawQuery = "INSERT INTO withdrawals (blood_type, record_date, blood_pints) VALUES ('$bloodType', '$recordDate', '$bloodPints')";
            $withdrawResult = mysqli_query($con, $withdrawQuery);

            if ($withdrawResult) {
                // Update the blood stock quantity
                $newQuantity = $availableQuantity - $bloodPints;
                $updateStockQuery = "UPDATE blood_stock SET quantity = '$newQuantity' WHERE blood_type = '$bloodType'";
                mysqli_query($con, $updateStockQuery);

                $_SESSION['withdrawSuccess'] = "Withdrawal successful.";
            } else {
                $_SESSION['withdrawError'] = "Unable to perform withdrawal. Please try again later.";
            }
        } else {
            $_SESSION['withdrawError'] = "Insufficient blood stock for withdrawal.";
        }
    } else {
        $_SESSION['withdrawError'] = "Blood type not found in stock.";
    }
